...
export default function NewsletterModal({
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem(localKey) === "1" || sessionStorage.getItem(sessionKey) === "1") return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setSubmitted(true);
        setEmail("");
        localStorage.setItem(`newsletterSubmitted:${countryCode || "global"}`, "1");
        const modalEl = document.getElementById("newsletterModal");
        if (modalEl) {
          const { Modal } = await import("bootstrap/dist/js/bootstrap.esm.min.js");
  ...
  return (
    ...
    >
      <div className="modal-dialog modal-dialog-centered modal-md">
        <div className="modal-content rounded-2 p-4" style={{ backgroundColor: "#e4f0f8" }}>
          <button
            type="button"
          ...
          {submitted ? (
            <p className="text-center text-success">Thank you for subscribing!</p>
          ) : (
            <>
              <form onSubmit={handleSubmit} className="d-flex flex-column gap-3 mb-3">
                ...
                <button type="submit" className="btn btn-dark btn-lg rounded-pill fw-bold">
                  Subscribe – It’s Free
                </button>
              </form>
              ...
              <div className="d-flex flex-column gap-2">
                    <button
                                className="bg-white shadow px-4 py-3 item-center btn gsigninbtn btn-outline-white w-100 mb-2"
                ...
                {/* <button className="btn btn-light border btn-lg rounded-pill text-start">
                  <img src="/icons/facebook.svg" alt="Facebook" style={{ height: 20, marginRight: 8 }} />
                  Sign Up with Facebook
                </button>
                <button className="btn btn-light border btn-lg rounded-pill text-start">
                  <img src="/icons/apple.svg" alt="Apple" style={{ height: 20, marginRight: 8 }} />
                  Sign Up with Apple
                </button> */}
              </div>
