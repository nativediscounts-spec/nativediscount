...
export default function CopyCode({ code }) {
  ...
  return (
    <div className="border-sunshine bg-sunshine-100 relative mb-4 flex items-center justify-between rounded border border-dashed p-3">
      <p
        className="w-48 break-words p-1 pr-3 text-center font-bold tracking-wide md:mx-auto md:w-auto md:pr-1"
        data-qa="el:code"
      >
        {code}
      </p>
      <button
        onClick={handleCopy}
        disabled={copied}
        className={`overflow-hidden duration-500 transition-all ease-out-quint focus:outline-none cursor-pointer font-bold rounded-lg flex items-center justify-center h-8 w-32 sm:px-3 ${
          copied
